...
const CreateAppointment = ({ appointments, setAppointments }) => {
    ...
    useEffect(() => {
        if (date) {
            ...
            const existingAppointments = appointments.filter(app => app.date === date);
            ...
            existingAppointments.forEach(app => {
                ...
                for (let diff = -30; diff <= 30; diff += 15) {
                    ...
                    if (conflictedMinutes >= 0 && conflictedMinutes <= 24 * 60) {
                        ...
                        const conflictedTime = `${conflictedHours.toString().padStart(2, '0')}:${conflictedMins.toString().padStart(2, '0')}`;
                        unavailableTimes.push(conflictedTime);
                    }
                }
            });

            const filteredTimes = allTimes.filter(t => !unavailableTimes.includes(t));
    ...
    const handleSearchChange = (e) => {
        ...
        if (term) {
            ...
            // Filter student IDs that include the search term
            const matchedIds = keys.filter((key) => key.toLowerCase().includes(term.toLowerCase()));

            // Update the matched student IDs
    ...
    const handleAdd = () => {
        const newAppointment = { studentId, date, time };
    ...
    const handleSelectStudentId = (id) => {
        setSearchTerm(id);
        setMatchedStudentIds([]);
    ...
    const handleSearch = (e) => {
        ...
        if (searchTerm) {
            const studentData = localStorage.getItem(searchTerm);
            if (studentData) {
                const student = JSON.parse(studentData);
                setSearchResult(student);
                setName(student.name);
    ...
    const handleSubmit = (e) => {
        ...
        const isConflict = appointments.some((appointment) => {
            if (appointment.date === date) {
                ...
                const diffInMs = Math.abs(existingAppointmentDateTime - newAppointmentDateTime);
        ...
        if (isConflict) {
            setMessage('The selected time slot is not available. Please choose a different time.');
            return;
        }
        ...
        if (isSearchMode) {
            ...
            localStorage.setItem(studentId, JSON.stringify(newStudent));
            setMessage('New student information saved.');
            setIsSearchMode(true);
            setSearchResult(newStudent);
                ...
                <div className="fixed inset-0 bg-gray-900 bg-opacity-50 flex items-center justify-center z-30">
                    <div className="bg-white rounded-lg shadow-xl p-7 w-full max-w-lg mx-4 md:mx-0 md:w-1/2">
                        ...
                        <div className="flex justify-center mb-4">
                            <button
                                className={`px-4 py-2 rounded-l-full ${isSearchMode ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700'}`}
                                onClick={() => setIsSearchMode(true)}
                            >
                                Search Mode
                            </button>
                            <button
                                className={`px-4 py-2 rounded-r-full ${!isSearchMode ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700'}`}
                        ...
                        <hr className="mb-4" />
                        {isSearchMode ? (
                            <form onSubmit={handleSearch} className="grid grid-cols-1 gap-1">
                                <div className="p-1 rounded-lg text-gray-700" ref={dropdownRef}>
                                    <label className="block mb-2 font-medium text-blue-800">Search Student ID</label>
                                    <div className="relative">
                                        ...
                                        {matchedStudentIds.length > 0 && (
                                            <ul className="absolute left-0 right-0 border border-blue-200 rounded-lg mt-1 max-h-40 overflow-y-auto bg-white z-50">
                                                {matchedStudentIds.map((id) => (
                                                    <li
                                                        key={id}
                                                        className="p-2 hover:bg-blue-100 cursor-pointer"
                                                        onClick={() => handleSelectStudentId(id)}
                                                    >
                                                        {id}
                                                    </li>
                                                ))}
                                ...
                                {searchResult && (
                                    <>
                                        ...
                                        <div className="flex justify-center space-x-4">
                                            <button type="button" className="bg-gray-600 text-white px-4 py-2 rounded-full hover:bg-gray-800 transition-colors duration-300" onClick={toggleMode}>
                                                Back
                                            </button>
                            ...
                            <form onSubmit={handleSubmit} className="grid grid-cols-1 gap-1">
                                ...
                                <div className="p-0.5 rounded-lg text-gray-700">
                                    <label className="block mb-2 font-medium text-blue-800">Major</label>
                                    <input
                                        type="text"
                                ...
                                <div className="p-0.5 rounded-lg text-gray-700">
                                    <label className="block mb-2 font-medium text-blue-800">Phone</label>
                                    <input
                                        type="text"
                                ...
                                <div className="flex justify-center space-x-4 mb-1">
                                    <button type="button" className="bg-gray-600 mb-1 text-white px-4 py-2 rounded-full hover:bg-gray-800 transition-colors duration-300" onClick={toggleMode}>
                                        Back
                                    </button>
                                    <button type="submit" className="bg-blue-600 mb-1 text-white px-4 py-2 rounded-full hover:bg-blue-800 transition-colors duration-300">
                                        Add
                                    </button>
