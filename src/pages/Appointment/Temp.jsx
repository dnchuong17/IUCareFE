...
const CreateAppointment = ({ appointments, setAppointments }) => {
  ...
  const handleSearchChange = async (e) => {
    ...
    if (value.length > 0) {
      try {
        const result = await api.searchPatient(value);
        if (result) {
    ...
    try {
      ...
      if (isSearchMode) {
        if (studentId && name) {
          ...
          throw new Error("Student ID or name is missing in search mode");
        }
      } else {
        ...
        const patientInfo = await getPatientInformation(patientResponse.id);
